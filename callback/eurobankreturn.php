...
if ($digest != $post_digest) {
    logTransaction($GATEWAY["name"], $_POST,
        "Unsuccessful Confirmation. Invalid Digest."); # Save to Gateway Log: name, data array, status
} else {
    ...
    if (isset($_GET['result']) && ($_GET['result'] == 'success')) {
        ...
        if ($status == 'CAPTURED') {
            ...
            if ($status == 'ERROR') {
                logTransaction($GATEWAY["name"], $_POST,
                    "Unsuccessful Confirmation. Unknown error."); # Save to Gateway Log: name, data array, status
            } else {
                if ($status == 'CANCELED') {
                    logTransaction($GATEWAY["name"], $_POST,
                        "Unsuccessful Confirmation. CANCELED"); # Save to Gateway Log: name, data array, status
                } else {//Failed Response codes
                    logTransaction($GATEWAY["name"], $_POST,
                        "Unsuccessful Confirmatio. $status"); # Save to Gateway Log: name, data array, status
                }
            }
        }
    } else {
        if (isset($_GET['result']) && ($_GET['result'] == 'failure')) {
            logTransaction($GATEWAY["name"], $_POST,
                "Unsuccessful Confirmation"); # Save to Gateway Log: name, data array, status
        }
    }
}
